...
def main(args):
    ...
    if args.split == "random":
        ...
        train_dataset, valid_dataset, _ = random_split(train_val_dataset, seed=args.seed, frac_train=0.85,
    ...
    if args.dataset == 'chem':
        ...
        train_loader = DataLoaderAE(train_dataset, batch_size=args.batch_size, shuffle=True,
                                    num_workers=args.num_workers)
        val_loader = DataLoaderAE(valid_dataset, batch_size=10 * args.batch_size, shuffle=False,
                                  num_workers=args.num_workers)
        if args.split == "species":
            # for species splitting
            test_easy_loader = DataLoaderAE(test_dataset_broad, batch_size=10 * args.batch_size, shuffle=False,
                                            num_workers=args.num_workers)
            test_hard_loader = DataLoaderAE(test_dataset_none, batch_size=10 * args.batch_size, shuffle=False,
                                            num_workers=args.num_workers)
        else:
            test_loader = DataLoaderAE(test_dataset, batch_size=10 * args.batch_size, shuffle=False,
                                       num_workers=args.num_workers)
    ...
    if not args.result_file == "":
        if args.dataset == 'bio' or args.dataset == 'dblp':
            with open("../res/" + args.dataset + '/' + "finetune_seed" + str(args.run_seed) + "/" + args.result_file,
                      'wb') as f:
                if args.split == "random":
                    pickle.dump(
                    ...
                    pickle.dump({"train": np.array(train_acc_list), "val": np.array(val_acc_list),
                                 "test_easy": np.array(test_acc_easy_list),
                                 "test_hard": np.array(test_acc_hard_list)}, f)

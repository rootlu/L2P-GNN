...
class MetaGINConv(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, edge_in_channels, **kwargs):
        ...
        # linear 1
        self.w1 = Parameter(torch.Tensor(2 * in_channels, out_channels))
    ...
    def forward(self, x, edge_index, edge_attr, w1, b1, bn_w, bn_b, w2, b2,
                # edge_emb1=None, edge_emb2=None,
                edge_encoder_w=None, edge_encoder_b=None):
        """"""
        ...
        edge_emb = F.linear(edge_attr.to(edge_encoder_w.dtype), edge_encoder_w, edge_encoder_b)
...
class MetaGIN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, weights=None):
        ...
        x = F.relu(self.conv1(x, edge_index, edge_attr,
                              ...
                              # weights['conv1.edge_embedding1.weight'], weights['conv1.edge_embedding2.weight']
                              ))
        ...
        x = F.relu(self.conv2(x, edge_index, edge_attr,
                              ...
                              # weights['conv2.edge_embedding1.weight'], weights['conv2.edge_embedding2.weight']
                              ))
        x = F.dropout(x, self.drop_ratio, training=self.training)
        x = F.relu(self.conv3(x, edge_index, edge_attr,
                              ...
                              # weights['conv3.edge_embedding1.weight'], weights['conv3.edge_embedding2.weight']
                              ))
        x = F.dropout(x, self.drop_ratio, training=self.training)
        x = F.relu(self.conv4(x, edge_index, edge_attr,
                              ...
                              # weights['conv4.edge_embedding1.weight'], weights['conv4.edge_embedding2.weight']
                              ))
        x = F.dropout(x, self.drop_ratio, training=self.training)
        x = self.conv5(x, edge_index, edge_attr,
                       ...
                       # weights['conv5.edge_embedding1.weight'], weights['conv5.edge_embedding2.weight']
                       )
        x = F.dropout(x, self.drop_ratio, training=self.training)
...
class MetaPool(torch.nn.Module):
    ...
    def forward(self, x, weights=None, batch=None):
        ...
        if batch is None:
            ...
            pooled_emb = global_mean_pool(x, batch)  # (num_graph, dim)  for finetune test
